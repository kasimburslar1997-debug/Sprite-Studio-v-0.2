...
export const Sprite = {
    ...
    exportGIF() {
        ...
        if (typeof window.gifshot === 'undefined') {
            alert('تعذر تحميل مكتبة GIF، يرجى التأكد من اتصال الإنترنت.');
            return;
        }
        ...
        const fps = parseInt(toEnDigits(this.fpsInput.value)) || 12;
        const srcW = Store.slicedFrames[0].width;
        const srcH = Store.slicedFrames[0].height;
        const targetW = Math.round(Math.min(srcW, 600));
        ...
        window.gifshot.createGIF({
            ...
            if (!obj.error) {
                ...
                console.error('GIF error:', obj.errorMsg || obj.error);
                alert('حدث خطأ أثناء إنشاء ملف الـ GIF');
            }
            hideProgress();
        });
    },
